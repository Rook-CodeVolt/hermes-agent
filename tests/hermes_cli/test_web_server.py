...
class TestWebServerEndpoints:
    ...
    def test_latest_descendant_never_resumes_into_a_subagent_or_branch_child(self):
        """Backport of upstream PR #117013 (fix commit b80292c9df, #115092):
        after a ws_orphan_reap the dashboard resumes the predecessor's newest
        descendant. A subagent run (``_delegate_from``) or a /branch fork
        (``_branched_from``) is its own conversation and never listed as a
        continuation, so following it parks the user's chat in a hidden row;
        only compression continuations are followed."""
        from hermes_state import SessionDB
        ...
        try:
            ...
            assert (
                self.client.get("/api/sessions/primary/latest-descendant").json()[
                    "session_id"
                ]
                == "primary"
            )
            ...
            db._conn.execute(
                "UPDATE sessions SET end_reason='compression' WHERE id='primary'"
            )
            db._conn.commit()
            db.create_session(
                session_id="primary-cont", source="tui", parent_session_id="primary"
            )
        finally:
            db.close()
