...
def _is_delegated_child_cli_mutation(args: argparse.Namespace) -> bool:
    ...
    #
    # Two complementary, independent signals are checked -- either being True
    # denies (deny-is-deny, OR-of-denials): real process ancestry (closes the
    # env-strip bypass, t_70827e4e / t_11e8c077) and path/root-scoped fencing
    # (lets a fenced child be scoped to a SPECIFIC board root rather than a
    # blanket deny-all).
    board_override = getattr(args, "board", None)
    try:
        from agent.delegation_context import is_delegated_child_process_context

        if is_delegated_child_process_context(board=board_override):
            return True
    except Exception:
        if bool(os.environ.get("HERMES_DELEGATED_CHILD_CONTEXT")):
            return True
