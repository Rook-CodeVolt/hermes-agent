...
def _assert_not_delegated_child_mutation(
    ...
    *,
    ...
    Two complementary, independent signals are checked -- either one being
    True is enough to deny (deny-is-deny, OR-of-denials):
    ...
    * *conn*/*board*: pass the actual connection/board slug this mutation is
      ...
      class (t_0977ea27 / t_85586891). This verifies real kernel process
      ancestry, closing the env-strip bypass (t_70827e4e / t_11e8c077).
    * *path* is the board DB / metadata root being mutated; ``None`` means the
      lineage's own board (``kanban_home()``). This checks path/root-scoped
      fencing (:func:`agent.delegation_context.kanban_path_is_fenced`), so a
      fenced child can be scoped to a SPECIFIC board root instead of a
      blanket deny-all.
    """
    ...
    if not delegated:
        delegated = kanban_path_is_fenced(kanban_home() if path is None else path)
    if delegated:
